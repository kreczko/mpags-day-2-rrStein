  ...
  for (size_type i{1}; i < nArgs; ++i) {
    ...
    if (args[i] == "-h" || args[i] == "--help") {
      ...
      if (i == nArgs - 1) {
        ...
        std::cout << "[WARNING] method specified incorrectly, defaulting to "
                     "only character transformation\n"
                  << std::endl;
      }
      ...
      if (i == nArgs - 1) {
        std::cerr
            << "[error] -key requires an integer (between 1 and 26) argument\n"
            << std::endl;
        return 1;
      } else if (x > 26 || x < 1) {
        std::cout
            << "[Warning] -key argument out of bounds, defaulting to key = 5\n"
            << std::endl;
        caesarKey = 5;
        ++i;
      } else {
